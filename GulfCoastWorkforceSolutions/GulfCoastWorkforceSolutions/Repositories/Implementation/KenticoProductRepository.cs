        ...
        public IEnumerable<SKUTreeNode> GetProductsByStatus(int statusId, int count = 0)
        {
            return pageRetriever.Retrieve<TreeNode>(
                query => query
                    ...
                    .FilterDuplicates(),
                cache => cache
                    .Key($"{nameof(KenticoProductRepository)}|{nameof(GetProductsByStatus)}|{statusId}|{count}")
        ...
        public IEnumerable<SKUTreeNode> GetProducts(string parentPageAliasPath, int count = 0)
        {
            return pageRetriever.Retrieve<TreeNode>(
                query => query
                    ...
                    .FilterDuplicates(),
                cache => cache
                    .Key($"{nameof(KenticoProductRepository)}|{nameof(GetProducts)}|{parentPageAliasPath}|{count}")
