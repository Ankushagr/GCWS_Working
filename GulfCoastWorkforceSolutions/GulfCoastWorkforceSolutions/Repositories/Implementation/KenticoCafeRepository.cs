...
namespace GulfCoastWorkforceSolutions.Repositories.Implementation
{
    /// <summary>
    ...
    public class KenticoCafeRepository : ICafeRepository
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="KenticoCafeRepository"/> class that returns cafes.
        /// </summary>
        /// <param name="pageRetriever">Retriever for pages based on given parameters.</param>
        public KenticoCafeRepository(IPageRetriever pageRetriever)
        {
        ...
        public IEnumerable<Cafe> GetCompanyCafes(string nodeAliasPath, int count = 0)
        {
            return pageRetriever.Retrieve<Cafe>(
                query => query
                    ...
                    .OrderBy("NodeOrder"),
                cache => cache
                    .Key($"{nameof(KenticoCafeRepository)}|{nameof(GetCompanyCafes)}|{nodeAliasPath}|{count}")
        ...
        public Cafe GetCafeByGuid(Guid guid)
        {
            return pageRetriever.Retrieve<Cafe>(
                query => query
                    ...
                    .TopN(1),
                cache => cache
                    .Key($"{nameof(KenticoCafeRepository)}|{nameof(GetCafeByGuid)}|{guid}"))
                .FirstOrDefault();
        }
        ...
        public IEnumerable<Cafe> GetPartnerCafes(string nodeAliasPath)
        {
            return pageRetriever.Retrieve<Cafe>(
              query => query
                  ...
                  .OrderBy("NodeOrder"),
              cache => cache
                  .Key($"{nameof(KenticoCafeRepository)}|{nameof(GetPartnerCafes)}|{nodeAliasPath}")
                  // Include path dependency to flush cache when a new child page is created or page order is changed.
                  .Dependencies((_, builder) => builder.PagePath(nodeAliasPath, PathTypeEnum.Children).PageOrder()));
        }
    }
