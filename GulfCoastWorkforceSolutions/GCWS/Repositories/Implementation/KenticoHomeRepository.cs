...
namespace GCWS.Repositories.Implementation
{
    /// <summary>
    ...
    public class KenticoHomeRepository : IHomeRepository
    {
        ...
        public Task<IEnumerable<Home>> GetHomeSectionsAsync(string nodeAliasPath, CancellationToken cancellationToken)
        {
            return pageRetriever.RetrieveAsync<Home>(
                query => query
        ...
        {
            return pageRetriever.Retrieve<Home>(
                query => query
                    .Path(nodeAliasPath, PathTypeEnum.Children)
                    .OrderBy("NodeOrder"),
                cache => cache
